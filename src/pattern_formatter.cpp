...
namespace spdlog::details {
    ...
    public:
        ...
        template<typename T>
        static unsigned int count_digits(T n) { return fmt_helper::count_digits(n); }

        ~scoped_padder() {
            if (remaining_pad_ >= 0) { pad_it(remaining_pad_); }
    ...
    static std::array days{"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
...
namespace spdlog {
    ...
    std::unique_ptr<formatter> pattern_formatter::clone() const {
        ...
        for (auto &it: custom_handlers_) { cloned_custom_formatters[it.first] = it.second->clone(); }
    ...
    void pattern_formatter::format(const details::log_msg &msg, memory_buf_t &dest) {
        ...
        for (auto &f: formatters_) { f->format(msg, cached_tm_, dest); }
        // write eol
    ...
    // Return padding.
    details::padding_info pattern_formatter::handle_padspec_(
    ...
    void pattern_formatter::compile_pattern_(const std::string &pattern) {
        ...
        for (auto it = pattern.begin(); it != end; ++it) {
            if (*it == '%') {
                ...
                if (it != end) {
                    if (padding.enabled()) { handle_flag_<details::scoped_padder>(*it, padding); }
                    else { handle_flag_<details::null_scoped_padder>(*it, padding); }
                }
                else { break; }
            }
