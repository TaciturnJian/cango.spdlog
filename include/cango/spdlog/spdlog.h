...
namespace spdlog {
    ...
    void log(source_loc source,
        level::level_enum lvl,
        format_string_t<Args...> fmt,
        Args &&... args) { default_logger_raw()->log(source, lvl, fmt, std::forward<Args>(args)...); }
    ...
    template<typename T>
    void log(source_loc source, level::level_enum lvl, const T &msg) { default_logger_raw()->log(source, lvl, msg); }

    template<typename T>
    void log(level::level_enum lvl, const T &msg) { default_logger_raw()->log(lvl, msg); }
...
inline void critical(wformat_string_t<Args...> fmt, Args &&...args) {
    ...
    template<typename T>
    void trace(const T &msg) { default_logger_raw()->trace(msg); }

    template<typename T>
    void debug(const T &msg) { default_logger_raw()->debug(msg); }

    template<typename T>
    void info(const T &msg) { default_logger_raw()->info(msg); }

    template<typename T>
    void warn(const T &msg) { default_logger_raw()->warn(msg); }

    template<typename T>
    void error(const T &msg) { default_logger_raw()->error(msg); }

    template<typename T>
    void critical(const T &msg) { default_logger_raw()->critical(msg); }
} // namespace spdlog
